...
//    }
    ...
    @KafkaListener(topics = "${kafka.listeningTopic8}" ,groupId = "${kafka.groupId}",
            containerFactory="${kafka.containerFactory}")
    public void consumeInterestJson(@Payload Section section) {
        logger.info(String.format("${kafka.consumed} : %s", section));

        List<String> interestList=new ArrayList<>();
        Chicklets[] chicklets=section.getChicklets();
        for(Chicklets chicklet:chicklets){

            String interest=chicklet.getInterest();
        ...
        try {
            ...
            logger.info(operation);
            if(operation.equals("add")) {
                employeeService.addEducationData(interList,userId );
            }else if(operation.equals("delete")){
                logger.info(interestList.get(0).toString());
                employeeService.deleteEducationData(educationList.get(0),userId);
            }else if(operation.equals("update")){
                employeeService.updateEducationData(educationList.get(0),userId);
            }else{
