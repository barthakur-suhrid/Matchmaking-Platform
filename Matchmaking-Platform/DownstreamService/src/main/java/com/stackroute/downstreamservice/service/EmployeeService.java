...
public class EmployeeService {
    ...
    //    Method for getting an employee from the Employees Document of MongoDB
    public void addInterest(List<String> interests,String employeeId) throws EmployeeNotFoundException {
        Employee employee;
        if (employeeRepository.existsById(employeeId)) {
            employee = employeeRepository.findById(employeeId).get();
            List<String> interest1=new ArrayList<>();
            if(employee.getInterest()==null){
                interest1=employee.getInterest();
            }

            for(String interest:interests){
                interest1.add(interest);
            }
            employee.setInterest(interest1);
        } else {
            throw new EmployeeNotFoundException(environment.getProperty("errors.emptyDatabase"));
        }
    }
